...
Command docs are the index into implementation: start at the linked doc, then follow its handler and compatibility breadcrumbs.

### Current Commands

| Command | Usage | What it does |
| --- | --- | --- |
| [`add-repo@1`](kb/cb7c5823-486f-52af-b3bc-0f368f277b0e.md) | `npx nosedive add-repo <repo-id-or-name> [--effort <effort>] [--ref <ref>] [--read-only] [--apply]` | Add a kb `kind: repo` doc to an effort's repo list, resolving the repo by id or exact name. |
...
| [`nuke@1`](kb/3570e756-f8e7-5e95-b911-09d7d116cd23.md) | `npx nosedive nuke --config` | Remove nosedive-managed bridge config files and managed config exclude blocks; refuses to run without `--config`. |
| [`pitch@1`](kb/f9325040-bb48-57f5-a98e-bfa0f2497661.md) | `npx nosedive pitch <slug> [--gist "<gist>"] [--pitch "<pitch>"] [--parent <parent>]` | Create a new effort file in `backlog/`, optionally nested under a parent effort, domain directory, or slug chain. |
...
| [`prove@1`](kb/af12dc22-6bad-5e2a-aca9-ff0163dd39dd.md) | `npx nosedive prove <assertion-uuid> [--record] [--verbose]` | Run an executable proof for a bridge `kind: assertion` doc in an isolated child process, optionally recording the proven input commits. |

...
| [`land@2`](kb/587d3f73-2534-5179-b111-ce6c83d6814d.md) | `npx nosedive land` | Canonicalize the active dive -- push each hydrated scope naming a work branch to that branch, close the dive as a memo with an outcome section, and leave each scoped worktree hydrated at the commit it pushed. |
...
| [`pack@2`](kb/9e28155b-b018-5e00-a3a1-707593167e1e.md) | `npx nosedive pack` | Capture all WIP on the active dive's scoped repos and bridge kb/ as patch artifacts, commit and push the bridge, release the dive to its packer, and reset every scoped worktree to its pin. |
...
| [`record.dive@2`](kb/c583519a-95c3-59e8-90fd-0d3003b4b8d1.md) | `npx nosedive record.dive [--ref <dive-ref>] [--feat <feat-ref>] [--effort <feat-ref>] [--gist <gist>] [--title <title>] [--brief <brief>] [--diver <email>] [--takeover] [--packer] [--scope <repo-ref>]... [--clear-scopes] [--upscope <repo-ref>]... [--unscope <repo-ref>]... [--work-branch <branch>] [--repin] [--free]` | Create or patch a feat-owned dive record while preserving omitted update fields. |
| [`record.gate@2`](kb/2c678acb-b4b8-5623-a900-12939da5d472.md) | `npx nosedive record.gate "<gist>" --feat <feat-ref> [--name <slug>] [--height <n>] [--flaky]` | Mint a test gate -- its doc, its stub script, and the link declaring it on a feat -- ready to fail until somebody writes the check. |

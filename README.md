...
A branch you can push is not what nosedive is for. Gates are: you say once what
...
nosedive test                       # with no dive on deck this sweeps the
                                    # backlog, finds the new gate red, and
                                    # mints a dive to fix it

nosedive record.dive --ref <that dive> --upscope BASE \
  --work-branch work/goodbye-note-exists
nosedive jump <that dive>

nosedive land                       # refused: the gate is still red

# write the check in kb/artifacts/<gate-id>.mjs, then satisfy it in
# workspace/__self and commit there

nosedive land                       # pushes work/goodbye-note-exists
```

A freshly minted gate fails on purpose. One that passed before anybody wrote it
would teach you that green means checked when it means unchecked.

The dive that sweep minted is held to that one gate and nothing else, so it
cannot land until the gate is green. Your gate script gets `ctx.repos`, keyed
by repo name, so it can look at the same worktree you are working in --

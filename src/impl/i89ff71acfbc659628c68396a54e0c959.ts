...
import { resolveActiveFeatDoc } from "../lib/repoFeatScopes.js";

function featAncestry(feat: KbDoc, kbDocs: KbDoc[]): KbDoc[] {
	const feats = [feat];
	const seen = new Set([feat.id]);
	let current = feat;
	for (;;) {
		const parentRef = current.links.find((link) => link.rel === "parent")?.id;
		if (!parentRef) return feats;
		const parent = kbDocs.find((doc) => doc.id === parentRef);
		if (!parent) throw new Error(`feat ${current.id} links missing parent feat ${parentRef}`);
		if (seen.has(parent.id)) throw new Error(`feat ancestry contains a cycle at ${parent.id}`);
		seen.add(parent.id);
		feats.push(parent);
		current = parent;
	}
...
function spin(args: string[], io: CommandIo): void {
	...
	const feat = resolveActiveFeatDoc(kbDocs, rc);
	const repos = new Map<string, KbDoc>();
	for (const ancestor of featAncestry(feat, kbDocs)) {
		for (const scope of ancestor.scopes) {
			const repo = kbDocs.find((doc) => doc.kind === "repo" && doc.id === scope.repoId);
			if (!repo) throw new Error(`feat ${ancestor.id} scopes missing repo ${scope.repoId}`);

 ...
 */
export function parseEffortRange(commandDocPath: string): EffortRange {
	...
	const minimum = effortScalar(meta, "minimum-effort", label);
	const maximum = effortScalar(meta, "maximum-effort", label);
	if (maximum < minimum) {
